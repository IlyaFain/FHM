...
$(function()
{
	...
	(function Slider()
	{
		...
		var
			...
			var firstRun = true;
			$('#slider_video_open').click(function()
			{
				if (!firstRun)
				{
					$('#slider').addClass('_open');
					return;
				}
				else
				{
					var template_youtube = _.template($('#template_slider_video').html());
					setTimeout(function()
					{
						firstRun = false;
						$('#slider_video').append(template_youtube()); 
						$('#slider_video_close').click(function()
						{
							$('#slider')
								.removeClass('_open')
								.find('iframe')[0].contentWindow.postMessage('{"event":"command","func":"pauseVideo","args":""}', '*');
						});
					}, 1000);
